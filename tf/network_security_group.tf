...
resource "azurerm_network_security_group" "common" {
  ...
  dynamic "security_rule" {
    ...
    content {
      ...
      destination_address_prefixes               = try(split("/", security_rule.value[6])[0] == "subnet" ? data.azurerm_subnet.subnets[split("/", security_rule.value[6])[1]].address_prefixes : (split("/", security_rule.value[6])[0] == "prefixes" ? local.prefixes[(split("/", security_rule.value[6])[1])] : null), null)
    }
  }
}
